...
void SDLStub::processEvents() 
{
	...
	static SDL_FingerID activeFinger = 0;
	...
	while (SDL_PollEvent(&ev)) 
	{
		...
		case SDL_MOUSEBUTTONDOWN:
			if (ev.button.button == SDL_BUTTON_LEFT) {
				input.button = true; // Run/shoot action
			}
			break;
		case SDL_MOUSEBUTTONUP:
			...
			// On mouse button release, stop movement
			input.dirMask = 0;
			break;
		case SDL_MOUSEMOTION:
		{
			// Map mouse movement to direction
			const int threshold = 4; // pixels
			input.dirMask = 0;
			if (ev.motion.xrel < -threshold) input.dirMask |= PlayerInput::DIR_LEFT;
			else if (ev.motion.xrel > threshold) input.dirMask |= PlayerInput::DIR_RIGHT;
			if (ev.motion.yrel < -threshold) input.dirMask |= PlayerInput::DIR_UP;
			else if (ev.motion.yrel > threshold) input.dirMask |= PlayerInput::DIR_DOWN;
		...
		case SDL_FINGERDOWN:
		{
			activeFinger = ev.tfinger.fingerId;
		...
		case SDL_FINGERUP:
		{
			if (ev.tfinger.fingerId == activeFinger) {
				...
				float dy = ev.tfinger.y - lastTouchY;
				float dist = dx * dx + dy * dy;
				const float tapDist = 0.0025f; // ~0.05*0.05 in normalized coords
				const Uint32 tapTime = 200; // ms
				if (!touchMoved && tapDuration < tapTime && dist < tapDist) {
					input.button = true; // Fast tap = run/shoot
				} else {
					input.button = false;
				}
				input.dirMask = 0;
		...
		case SDL_FINGERMOTION:
		{
			if (ev.tfinger.fingerId == activeFinger) {
				...
				float dy = ev.tfinger.y - lastTouchY;
				input.dirMask = 0;
				if (dx < -threshold) input.dirMask |= PlayerInput::DIR_LEFT;
				else if (dx > threshold) input.dirMask |= PlayerInput::DIR_RIGHT;
				if (dy < -threshold) input.dirMask |= PlayerInput::DIR_UP;
				else if (dy > threshold) input.dirMask |= PlayerInput::DIR_DOWN;
				touchMoved = (input.dirMask != 0);
			}
		}
		break;
